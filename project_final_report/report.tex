...
\subsection{Models and task-specific evaluator}
The spatial VAE uses residual convolutional blocks, GroupNorm, SiLU, two downsampling stages, a $4\times8\times8$ Gaussian latent, and symmetric nearest-neighbor upsampling. Pixel DDPM and LDM use two-resolution time-conditioned U-Nets with sinusoidal embeddings, residual blocks, skip connections, bottleneck self-attention, and EMA weights. Figure~\ref{fig:architecture} compares their generation paths. The frozen evaluator has 64/128/256-channel residual stages and a 256-dimensional feature vector, following the optimization motivation of residual learning \citep{he2016resnet}.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.90\linewidth]{figures/model_architectures.png}
  \caption{Sampling paths. VAE decodes once; DDPM denoises pixels for 100 steps; LDM denoises a spatial latent for 100 steps and then decodes.}
...
Complete-system parameter counts are 1.955M, 7.743M, and 5.787M for VAE, LDM, and DDPM. Despite having 33.8\% more parameters than DDPM because its total includes the autoencoder, LDM has 32\% lower measured peak memory: 5.20--5.21 GB versus 7.68--7.70 GB. VAE peaks at 6.30--6.32 GB. This inversion shows that the 4,096-sample activation geometry, rather than weights alone, dominates high-throughput inference memory. All 30 runs have finite losses and no detected collapse. Appendix~\ref{app:diagnostics} provides convergence, per-seed FID, and separate training/sampling memory plots.
...
\begin{minipage}{0.56\linewidth}
  ...
  \small (a) Mean $\pm$ SD training curves
\end{minipage}\hfill
\begin{minipage}{0.42\linewidth}
  \includegraphics[width=\linewidth]{figures/seed_variability.png}
  \small (b) Seed-level FID
\end{minipage}
\caption{Optimization convergence and run-to-run variability. Loss magnitudes are not comparable across objectives.}
\end{figure}
