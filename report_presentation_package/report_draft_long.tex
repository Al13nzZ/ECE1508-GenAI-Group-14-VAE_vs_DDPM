...
\begin{abstract}
This project compares a convolutional Variational Autoencoder (VAE) and a Denoising Diffusion Probabilistic Model (DDPM) for image generation on Fashion-MNIST across ten independent random seeds. Both models were trained on the same dataset and evaluated with a shared pipeline using 5,000 generated samples per model per seed, classifier-based feature metrics, class-distribution statistics, and measured computational cost. Mean results with sample standard deviations show that the DDPM produces more recognizable samples and achieves substantially lower classifier-feature FID and KID than the VAE, with a more balanced predicted class distribution. In contrast, the VAE requires much less training time and generates samples through a single decoder pass. The ten-seed results establish a repeatable quality--efficiency trade-off while exposing meaningful run-to-run variability, particularly for DDPM distribution metrics.
\end{abstract}
...
Figure~\ref{fig:samples} provides a representative qualitative comparison from one run. Both models learn recognizable clothing shapes, but the VAE samples are generally blurrier and some outlines are less distinct. The DDPM samples have clearer edges and more recognizable object structure. Images cannot be averaged meaningfully across seeds, so qualitative figures are labeled as representative examples; all quantitative figures below aggregate ten runs.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.96\linewidth]{figures/main/real_vae_ddpm_comparison.png}
  \caption{Representative real Fashion-MNIST images and generated samples from one run. This figure illustrates appearance but is not used to estimate cross-seed variability.}
  \label{fig:samples}
\end{figure}
...
\end{minipage}
\caption{Representative generation structures from one run: VAE latent interpolation and DDPM reverse denoising. These procedural examples are not average images.}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/multiseed/quality_metrics_multiseed.png}
  \caption{Generation-quality metrics across ten independent random seeds. Bars show means and error bars show sample standard deviations. Arrows indicate the preferred direction. The agreement across classifier-feature FID, KID, confidence, recognizability, entropy, and sharpness supports the DDPM quality advantage while exposing its run-to-run variability.}
  \label{fig:quality_multiseed}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{figures/multiseed/compute_metrics_multiseed.png}
\caption{Computational cost across ten independent random seeds. Bars show means and error bars show sample standard deviations. Peak allocation is constant because architecture, batch size, and precision are fixed. These measurements characterize this RTX 5070 Ti implementation rather than hardware-independent constants.}
...
\section{Conclusions}
We implemented and evaluated a convolutional VAE and a DDPM on Fashion-MNIST under a shared experimental pipeline repeated across ten seeds. The DDPM consistently produced better mean classifier-feature distribution scores, recognizability, sharpness, and class balance, while the VAE was much more computationally efficient, especially during sampling. Future work should add paired statistical tests and confidence intervals, compare more closely matched parameter counts, and evaluate fewer DDPM sampling steps.
...
\end{minipage}
\caption{Representative training curves from one run. Aggregate final-loss statistics are reported in the main text; absolute VAE and DDPM losses are not directly comparable because the objectives differ.}
\end{figure}
...
\end{minipage}
\caption{Representative feature-space nearest-neighbor examples from one run. The main text reports mean memorization-screen rates across ten seeds. Visual similarity is expected for simple Fashion-MNIST classes and does not by itself establish copying.}
\end{figure}
